"""
Module providing utilities for reading and writing data to the database.

Provides DatabaseProvider, an essential component for manipulating the SQLite database, used
for storing data for automation.
"""
...
class DatabaseProvider:
    """
    Abstraction layer for accessing data from the SQLite database.
    """
    ...
    def db_connect(self):
        ...
        try:
            yield cur
            conn.commit()        # only runs if no exception
        except Exception:
            conn.rollback()      # ensures DB stays consistent
            raise
        finally:
            cur.close()
            conn.close()
    ...
    def _faulty_data_correction(self):
        ...
        with self.db_connect() as cursor:
            ...
            cursor.execute(
                "SELECT answer_string_1, answer_string_2, answer_string_3, answer_string_4, correct_answer_index, question_hash FROM questionData WHERE list_id = ? AND target_word=''",
                (self.current_list,),
            )
            ...
            cursor.execute(
                "SELECT id, answer_text, question_id FROM answers WHERE list_id = ?",
                (self.current_list,),
            )
            rows = cursor.fetchall()
            ...
            for row in rows:
                ...
                if answer_text not in correct_answers:
                    print(
                        f"-- [internal] potential faulty answer: {answer_text} (row id: {row_id})"
                    )
            ...
            cursor.execute(
                "SELECT id, question_text FROM questions WHERE list_id = ?",
                (self.current_list,),
            )
            ...
            for row in potential_faulty_answers:
                ...
                if question_id not in existent_question_ids:
                    print(
                        f"-- Faulty answer: {answer_text} in {row_id} linked with non-existent QID: {question_id}"
                    )
    ...
    def _store_question_answer(self, cursor: sqlite3.Cursor, question_hash: str, question_answer: str):
        assert self.current_list != -1
        
        print(f"store q answer hash {question_hash} answer {question_answer}")
        cursor.execute("INSERT OR IGNORE INTO questionAnswers (list_id, question_hash, answer) VALUES (?, ?, ?)", (self.current_list, question_hash, question_answer))
    ...
    def _get_answer_reputation_lookup(
        ...
        query_impossible = (
            "SELECT id, answer_reputation FROM answers WHERE 0"  # ensure impossibility
        )
        query_check_target_word = (
            "SELECT target_word FROM answers WHERE answer_text = ? AND list_id = ?"
        )
        query_check_target_word_strict = "SELECT target_word FROM answers WHERE answer_text = ? AND list_id = ? AND target_word = ?"

        # check if anything matches strict query
        cursor.execute(
            query_check_target_word_strict,
            (answer_text, self.current_list, target_word),
        )
        row = cursor.fetchone()
        if row:
        ...
        if row:
            ...
            if existing_target == target_word:
                print(
                    f"ANSWER REPUTATION LOOKUP: migrated and match: {existing_target} {target_word} {answer_text} {target_word}"
                )
                # strict match
                return True, query_strict, (answer_text, self.current_list, target_word)
            if existing_target == "":
            ...
            if existing_target != target_word:
                print(
                    f"ANSWER REPUTATION LOOKUP: does not match {existing_target} {target_word} {answer_text}"
                )
        ...
        print(
            f"ANSWER REPUTATION LOOKUP: no query found for: {query_check_target_word} with answer: {answer_text} in {self.current_list}"
        )
...
store question:
  ...
  list_id = {self.current_list}
              """)
        with self.db_connect() as cursor:
            self._store_question_answer(cursor, question_hash, answer_text)
            ...
            if not row:
                print(f"Unable to store answer: question ID not found where hash is: {question_hash} and list ID is: {self.current_list}")
                return
    ...
    def lookup_answer(self, question_hash: str) -> str:

        assert self.current_list != -1

        print(f"lookup answer where question_hash = {question_hash}")
        with self.db_connect() as cursor:

            cursor.execute(
                "SELECT answer FROM questionAnswers WHERE question_hash = ? AND list_id = ?",
                (question_hash, self.current_list),
            )
